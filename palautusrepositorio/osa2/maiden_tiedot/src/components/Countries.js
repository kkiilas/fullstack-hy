...
const Countries = ({ countries, filter, showCountries, nonsingularCapital,
  handleShowClick, handleNonsingularCapitalClick, weather }) => {

  const filteredCountries = showCountries
    ? countries.filter(({ name }) => name.common.toLowerCase().includes(filter.toLowerCase()))
    ...
    : (filteredCountries.length > 10)
      ...
      : (filteredCountries.length > 1)
        ? filteredCountries
          ...
          .map((country, i) =>
            <Country key={i} name={country.name.common} handleShowClick={handleShowClick} />)
        : <CountrySummary
